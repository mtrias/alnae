...
La condición de ortogonalidad establece que el producto punto de cada columna de $A$ con el error debe ser cero: $\trans{A} \cdot e = 0$. Entonces $\trans{A} \cdot (b - A\hat{x}) = 0$. Distribuyendo los términos, obtenemos las **Ecuaciones Normales**:
...
La solución del problema de mínimos cuadrados tiene una interpretación geométrica elegante: estamos buscando el punto en el subespacio definido por el espacio columna de $A$ que se encuentra a la distancia mínima del vector de datos $b$ [@lecture Lec. 9; @strang II.2; @alnae Clase 17].
...
Podemos expresar la proyección $p$ directamente a partir de $b$ mediante una matriz $P$ tal que $p = Pb$. Al sustituir la solución de las ecuaciones normales (@eq-normales) en $p = A\hat{x}$, obtenemos:
...
3.  **Caso Ortonormal:** Si las columnas de $A$ son ortonormales ($A = Q$), la matriz de proyección se simplifica drásticamente a $P = QQ^\top$ [@alnae Clase 5; @lecture Lec. 3].
