...
Prueba
: Como la matriz $S$ es simétrica, el teorema espectral nos dice que podemos escribirla como $Q\Lambda Q^\top$. Tomamos $Y=\sqrt{\Lambda}$, entonces $S=Q\sqrt{\Lambda}\sqrt{\Lambda}Q^\top = YY^\top$. [@practico Pr4 Ej1]

El recíproco se prueba en la @sec-matrices-gram
...
### Matrices de Gram<span class="toc-ignore"> ($A^\top A$)</span> {#sec-matrices-gram}
...
Prueba
: La forma cuadrática se reduce a la norma al cuadrado del vector transformado, y ésta es siempre no negativa.
$$x^\top (A^\top A) x = (Ax)^\top (Ax) = \|Ax\|^2 \geq 0$$
