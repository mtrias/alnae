...
Esta sección contiene una breve referencia sobre conocimientos previos, necesarios para entender el contenido del curso
...
: Basada en la identidad de Euler: $e^{i\theta} = \cos \theta + i \sin \theta$ [^complex-euler-obs], esta es la forma más compacta y potente para el análisis: $z = r e^{i\theta}$

[^complex-euler-obs]:
  De la formula de Euler $e^{i\theta} = \cos\theta + i\sin\theta$, evaluando en $\theta=\pi$ surge
  $$e^{i\pi}+1=0$$
  En consecuencia, $e^{i\pi}$ parametriza todos los puntos de $S^1={z\in\mathbb{C}:|z|=1}$
...
: Se eleva el módulo y se multiplica el argumento
